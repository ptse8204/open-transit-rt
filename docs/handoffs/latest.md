...
signed-in session banner and Login & Sessions status page. Phase 166 reshapes
`/admin/operations` into a dashboard-first landing page with top-three issues,
compact category summaries, and longer workflow details behind disclosures.
Phase 167 refactors the setup wizard into ten skippable GET-only steps and
adds a persistent dashboard reminder while setup remains incomplete. Phase 168
...
remain separate later gates. Phase 171 adds connector dry-run job records and
...
enabled by the browser. Phase 174 adds Validator Setup at
`/admin/operations/connectors/validators` and
`/admin/operations/connectors/validators.json`. The page supports
`static-mobilitydata` and `realtime-mobilitydata` allowlisted validator IDs,
...
Phase 177 adds required, recommended, and optional setup completion buckets.
The dashboard reminder stays tied to incomplete setup, can be dismissed only
for the current browser session/current next setup blocker, and returns when
...
Phase 179 adds local release-candidate diagnostic rows for the production auth
boundary, password login, first-admin bootstrap link behavior, logout, cookie
CSRF rejection, dashboard issue priority, setup wizard skip/reminder behavior,
connector examples versus configured instances, and connector dry-run
redaction. The release-candidate helper remains private local diagnostics
only: it does not write retained evidence, contact external parties, change
...
CSRF remain forbidden, and the public edge remains feed-only. SSO/OIDC is
explicitly deferred and must later map external identity into the same internal
signed `admin_session` session model.
...
CP000008 are complete for GitHub Pages and agency UI product polish: CP000008
reconciled and published the actual `gh-pages` branch at commit `a8b250e`,
GitHub Pages product story is refreshed, private Operations Console first-run
...
The exact next recommendation is maintainer review of the Phase 74 CP000008
...
CP000001 through CP000008 are complete for GitHub Pages and agency UI product
polish. CP000008 reconciled and published the actual `gh-pages` branch at
commit `a8b250e`. GitHub Pages product story is refreshed. Private Operations
