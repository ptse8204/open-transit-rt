...
| --- | --- | --- | --- |
| Check feed health | `/admin/operations/feed-health` | All five public paths, recorded HTTP status, byte count, checksum, validator state, health state, next action. | The page says public checks are missing, stale, blocked, or a proxy/server route needs repair. |
...
| Review public feed URLs | `/admin/operations/feed-health` | The five public paths still match the expected root and return current artifacts. | DNS, TLS, reverse proxy, or off-host checks need repair. |
...
- Ready: the private source record is present and currently looks usable.
