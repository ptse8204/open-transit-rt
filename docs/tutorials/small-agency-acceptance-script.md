...
Open the local app URL printed by the command, normally:
...
| --- | --- | --- | --- | --- | --- |
| Open the local app URL | The local app responds and routes to available public/admin paths. | Docker is not running, port `8080` is busy, or the local proxy did not start. | Run `make agency-app-logs` and `scripts/bootstrap-dev.sh --check`. | `docker compose -f deploy/docker-compose.yml ps` | Local startup does not prove hosted service availability. |
| Open the Operations Console at `/admin/operations` | The private console loads and shows **Start Here** near the top. | Missing or wrong local admin token, or app startup did not finish. | Use the token instructions printed by `make agency-app-up` and retry. | `make agency-app-logs` | Console access does not prove public launch or agency approval. |
| Review the Agency Operations Cockpit | The page shows setup progress, primary action cards, current signals, next actions, and what each card does not prove. | Older checkout or browser cache. | Refresh and confirm the branch includes Phase 71 UI changes. | `/admin/operations.json` | Cockpit visibility does not prove adoption, compliance, hosted service, or production readiness. |
| Confirm Start Here / launchpad path is visible | The page shows no-developer and developer paths, ordered first-run tasks, and five feed URLs. | Browser cache or an older checkout. | Refresh and confirm the branch includes Phase 69 UI changes. | `go test ./cmd/agency-config -run FirstRun` | Visibility does not prove evidence intake or compliance. |
...
| Check `/public/feeds.json` | The local feed discovery document is reachable from the local app. | Local app not started or publication metadata missing. | Return to Start Here and Feeds page. | `curl http://localhost:8080/public/feeds.json` | Local discovery does not prove source-of-truth website listing. |
...
| Open Help | `/admin/operations/help` explains GTFS, GTFS-RT, connectors, readiness, validators, telemetry, and claims/evidence. | None if the Operations Console is available. | Use Help links to return to the relevant UI page. | `/admin/operations/help.json` | Help is read-only guidance, not retained evidence. |
