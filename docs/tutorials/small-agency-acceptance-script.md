...
| Open the Operations Console at `/admin/operations` | The private console loads and shows **Agency Operations Cockpit / Start Here** near the top. | Missing or wrong local admin token, or app startup did not finish. | Use the token instructions printed by `make agency-app-up` and retry. | `make agency-app-logs` | Console access does not prove public launch or agency approval. |
| Review Agency Operations Cockpit / Start Here | The page shows setup progress, primary action cards, current signals, next actions, and what each card does not prove. | Older checkout or browser cache. | Refresh and confirm the branch includes Phase 71 UI changes. | `/admin/operations.json` | Cockpit visibility does not prove adoption, compliance, hosted service, or production readiness. |
| Confirm first-run launchpad tasks are visible | The page shows no-developer and developer paths, ordered first-run tasks, and five feed URLs. | Browser cache or an older checkout. | Refresh and confirm the branch includes Phase 69 UI changes. | `go test ./cmd/agency-config -run FirstRun` | Visibility does not prove evidence intake or compliance. |
...
| Check `/public/feeds.json` | The local feed discovery document is reachable from the local app. | Local app not started or publication metadata missing. | Return to Agency Operations Cockpit / Start Here and Feeds page. | `curl http://localhost:8080/public/feeds.json` | Local discovery does not prove source-of-truth website listing. |
