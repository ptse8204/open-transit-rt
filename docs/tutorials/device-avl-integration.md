...
| Trip Updates withheld | Vehicle Positions may exist, but Trip Updates output is empty or diagnostic counts show withheld reasons. | Trip Updates adapter conservatively withholds weak, stale, degraded, unsupported, or unmatched cases. | Review `/admin/operations/feed-health` Trip Updates quality diagnostics and `/admin/operations/telemetry` freshness. | Improve telemetry freshness, assignment confidence, GTFS data, and operational overrides. | Do not claim production-grade ETA quality. |
