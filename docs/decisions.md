...
`internal/state.Engine` is the only valid production matcher entry point. It requires schedule and assignment repositories, and construction fails fast if those dependencies are missing. The old placeholder rule-based matcher path was removed so placeholder feed code cannot accidentally look like production matching behavior.
