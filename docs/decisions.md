...
Active manual overrides are absolute in Phase 2 and are evaluated before stale telemetry fallback. Block-transition scoring requires same block, temporal plausibility, and plausible next-trip sequencing when start-time identity is available. Explicit telemetry bearing presence is distinct from numeric value, so `bearing: 0` is a valid true-north bearing. Persisted shape distance preserves `0` as a valid value. Repeated identical degraded unknown states reuse the active degraded assignment instead of creating redundant incidents.

The Phase 2 handoff is expected to describe actual implemented matcher behavior, not aspirational behavior. After the priority-fix pass, the handoff and implementation are aligned on constructor behavior, override precedence, degraded-state handling, system-failure taxonomy, batching, and post-midnight service-day limits.
