...
| External predictor adapter was evaluated | The repo documents the external predictor adapter boundary and candidate-style external predictor fit. | [Integration Adapter Kit](integration-adapter-kit.md), [Connector Catalog](connectors/catalog.md), and local adapter conformance checks. | The project runs TheTransitClock in production, proves production-grade ETA quality, or is endorsed by an external predictor project. |
...
| Demo/docs site plan exists | The repo has a repository documentation plan for a future public-friendly reader path. | [Demo And Documentation Site Plan](demo-docs-site-plan.md), [Documentation Home](README.md), [Wiki Home](../wiki/README.md). | A website was hosted, announced, publicly launched, agency-approved, consumer-accepted, compliance-backed, or production-ready. |
