...
Track B now inserts Phase 29A — External Predictor Adapter Evaluation and Phase 29B — AVL / Vendor Adapter Pilot Implementation after Phase 29 and before Phase 30. These inserted phases are planned only. They do not implement external predictor runtime integration, vendor AVL runtime integration, dependency status changes, consumer-status changes, public feed URL changes, GTFS-RT contract changes, or stronger ETA/compliance/vendor-support claims.
...
**Active phase:** Phase 29 — Real-World Realtime Quality Expansion is complete for the synthetic replay evidence expansion scope. Phase 29A — External Predictor Adapter Evaluation is the recommended next implementation phase. Phase 29B — AVL / Vendor Adapter Pilot Implementation follows Phase 29A, and Phase 30 — Consumer Submission Execution remains later. Track A — External Proof And Adoption is complete for the documented docs-only operator workflow, evidence intake, artifact-directory, and agency-domain readiness scope. Phases 12 through 29 remain closed for their documented scopes.
...
Start Phase 29A when maintainers are ready to continue Track B implementation. Recommended first slice: evaluate optional external prediction adapter feasibility, fallback behavior, and replay comparison while keeping deterministic prediction as the default and avoiding external runtime dependency or ETA-quality claims. Phase 29B should follow with a synthetic AVL/vendor adapter pilot pattern, and Phase 30 consumer submission execution remains later.
