...
| Mobility Database, Transitland | Workflow targets and discovery-readiness references only | Documented as possible publication/aggregator targets; current work is `/public/feeds.json`, stable URL, license/contact, and prepared-only review. No API integration, portal automation, submission, listing, or acceptance proof. |
| OpenTripPlanner | Future optional feed-consumer compatibility check | Not integrated. Future work may run offline GTFS/GTFS-RT compatibility checks with synthetic/local feeds. No OTP support or compatibility claim. |
| OneBusAway | Future optional compatibility or adapter investigation | Not integrated. Future work may evaluate OBA-oriented compatibility ideas behind explicit adapters. No OBA support or compatibility claim. |
| SIRI / GTFS-RT bridge | Future optional realtime bridge candidate | Not integrated. Any later work must be a sidecar/adapter that keeps raw private SIRI-like payloads out of the repo and converts only into bounded Open Transit RT DTOs. |
| GTFS-Flex, GTFS-ride, GTFS-Pathways, GTFS-Fares v2 | Future optional QA/readiness investigations | Not integrated. Future work must stay within GTFS QA/readiness boundaries and must not become dispatch, booking, fare payment, or unrelated analytics software. |
