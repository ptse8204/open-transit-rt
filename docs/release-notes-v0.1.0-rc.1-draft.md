...
- Source archive checksum: see attached release package checksum manifest.
...
- Phase 115 local package tooling must keep outputs under ignored `.cache`
  directories and must not write protected evidence paths.
- The local source archive is generated by `git archive HEAD`. The root
  `.gitattributes` export policy excludes protected retained-evidence and
  consumer-submission paths from generated source archives. The gate must still
  scan the final archive before publication.
...
| Source archive public-distribution review | gate required | Phase 115 adds an export-ignore policy; final package archive must scan with zero protected-path entries before publication. | Master Agent |
