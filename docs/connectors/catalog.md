...
| Generic JSON transform adapter | `cmd/avl-vendor-adapter --dry-run` with `testdata/avl-vendor` | `go test ./cmd/avl-vendor-adapter` | Vendor compatibility, real AVL reliability, production readiness, consumer acceptance, compliance, or ETA quality. |
