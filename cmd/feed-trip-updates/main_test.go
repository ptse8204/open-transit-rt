...
func TestTripUpdatesPathRoutedPublicFeedBuildsRequestedAgencyOnly(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/public/agencies/agency-a/gtfsrt/trip_updates.pb?agency_id=agency-b", nil)
	rr = httptest.NewRecorder()
	handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("agency-a with conflicting query status = %d, want 200: %s", rr.Code, rr.Body.String())
	}
	if builder.agencyCalls["agency-a"] != 2 || builder.agencyCalls["agency-b"] != 1 || builder.defaultCalls != 0 {
	...
	for _, path := range []string{
		"/public/agencies/agency%2Fbad/gtfsrt/trip_updates.pb",
		"/public/agencies/agency%5Cbad/gtfsrt/trip_updates.pb",
	} {
