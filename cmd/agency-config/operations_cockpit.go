...
func buildOperationsCockpit(page operationsPage) operationsCockpitView {
	progress := []operationsCockpitRow{
		cockpitProgressRow("agency_metadata", "Agency metadata", cockpitMetadataStatus(page), cockpitMetadataSignal(page), "Review agency name, public URL, license, and technical contact before sharing feed URLs.", "/admin/operations/setup", "This private view does not show agency approval or final-root ownership."),
		cockpitProgressRow("gtfs_imported", "GTFS imported", cockpitActiveFeedStatus(page), cockpitActiveFeedSignal(page), "Import or review the active schedule, then open GTFS quality and feed health.", "/admin/operations/gtfs-import", "This private view does not show schedule correctness or canonical validator success."),
		cockpitProgressRow("active_feed_version", "Active feed version", cockpitActiveFeedStatus(page), firstNonEmpty(page.ActiveFeedVersion, "no active schedule feed version recorded"), "Keep the active version visible before validator, rollback, or support decisions.", "/admin/operations/gtfs-import", "This private view does not show staged rollback execution is available in the browser."),
		cockpitProgressRow("five_feed_paths", "Five expected feed paths", cockpitFeedHealthStatus(page), cockpitFeedHealthSignal(page), "Open feed health and review each configured path, validator state, health state, and next action.", "/admin/operations/feed-health", "This private view does not show consumer acceptance, final-root ownership, or compliance."),
		cockpitProgressRow("validators", "Validators", cockpitValidationStatus(page), fmt.Sprintf("overall=%s; tooling=%s", page.ValidationHealth.OverallStatus, page.ValidationHealth.ToolingStatus), "Run or review allowlisted validator health from the private page.", "/admin/operations/validation-health", "This private view does not show validator-clean production data or consumer acceptance."),
		cockpitProgressRow("telemetry", "Telemetry", cockpitTelemetryStatus(page), telemetryEvidence(page), "Create or review device bindings, then send a safe sample telemetry event if needed.", "/admin/operations/devices", "This private view does not show real device reliability or vendor compatibility."),
		cockpitProgressRow("readiness", "Readiness", cockpitReadinessStatus(page), fmt.Sprintf("%d readiness rows; overall validation=%s", len(page.ReadinessV2.Rows), page.ValidationHealth.OverallStatus), "Open readiness for plain-language capability gaps and claim boundaries.", "/admin/operations/readiness", "This private view does not show CAL-ITP/Caltrans compliance."),
		cockpitProgressRow("maintenance", "Maintenance", maintenanceStatusToCockpit(page.Maintenance.OverallStatus), page.Maintenance.OverallStatus, "Review maintenance tasks, backup/restore configuration state, telemetry freshness, and support summary instructions.", "/admin/operations/maintenance", "This private view does not show SLA, uptime, hosted service availability, or production readiness."),
	}

	cards := []operationsCockpitCard{
		cockpitCard("import_update_gtfs", "Import Schedule", cockpitActiveFeedStatus(page), cockpitActiveFeedSignal(page), "Upload a GTFS ZIP or use a safe URL, then review quality and feed health.", "/admin/operations/gtfs-import", []string{"docs/tutorials/no-cli-agency-first-run.md"}, "This private view does not show source schedule correctness, agency approval, or compliance."),
		cockpitCard("review_feed_health", "Check feeds", cockpitFeedHealthStatus(page), cockpitFeedHealthSignal(page), "Review the five public feed paths and fix missing, stale, or invalid rows first.", "/admin/operations/feed-health", []string{"docs/tutorials/no-cli-agency-first-run.md", "docs/deployment/off-host-validation.md"}, "This private view does not show consumer acceptance, final-root ownership, or compliance."),
		...
		cockpitCard("synthetic_telemetry", "Try synthetic telemetry", telemetrySimulatorStatus(page), telemetrySimulatorSignal(page), "Preview local synthetic scenarios, then ask a helper for intentional sends when needed.", "/admin/operations/telemetry-simulator", []string{"docs/tutorials/telemetry-simulator-and-device-trial.md"}, "This private view does not show real service telemetry quality or real vendor compatibility."),
		cockpitCard("realtime_feed_state", "Review realtime", realtimeStatus(page), realtimeSignal(page), "Check stale vehicles, Vehicle Positions, withheld Trip Updates, and Alerts state.", "/admin/operations/realtime", []string{"docs/requirements-trip-updates.md"}, "This private view does not show production-grade ETA quality."),
		cockpitCard("manage_alerts", "Manage alerts", alertStatus(page), feedEvidence(page, "alerts"), "Use the Alerts Console for lifecycle work, then recheck the Alerts feed.", "/admin/alerts/console", []string{"docs/requirements-calitp-compliance.md"}, "This private view does not show consumer display or agency approval."),
...
func cockpitActionQueue(page operationsPage) []operationsCockpitAction {
	return []operationsCockpitAction{
		cockpitAction("start_setup", "Start setup", cockpitMetadataStatus(page), cockpitMetadataSignal(page), "Start setup", "/admin/operations/setup-wizard", "Ask an administrator only for deployment settings or real access-control changes.", "This private view does not show agency approval or final-root ownership."),
		cockpitAction("import_gtfs", "Import Schedule", cockpitActiveFeedStatus(page), cockpitActiveFeedSignal(page), "Import Schedule", "/admin/operations/gtfs-import", "Ask an administrator for source data fixes, large imports, or rollback.", "This private view does not show schedule correctness or validator-clean status."),
		cockpitAction("check_feeds", "Check feeds", cockpitFeedHealthStatus(page), cockpitFeedHealthSignal(page), "Check feeds", "/admin/operations/feed-health", "Ask an administrator for proxy, DNS, TLS, validator tooling, or off-host checks.", "This private view does not show consumer acceptance or compliance."),
		cockpitAction("connect_vehicles", "Connect vehicles", deviceStatus(page), deviceEvidence(page), "Connect vehicles", "/admin/operations/devices", "Ask an administrator for device secrets, GPS/AVL adapters, simulator sends, or real connector setup.", "This private view does not show vendor compatibility or production AVL reliability."),
		cockpitAction("review_realtime", "Review realtime", realtimeStatus(page), realtimeSignal(page), "Review realtime", "/admin/operations/realtime", "Ask an administrator when telemetry, matching, or prediction-adapter diagnostics are unclear.", "This private view does not show production-grade ETA quality or consumer display."),
		cockpitAction("fix_issues", "Fix issues", cockpitIssueStatus(page), cockpitIssueSignal(page), "Fix issues", "/admin/operations/gtfs-quality", "Ask an administrator when validator tooling, source GTFS, or deployment settings must change.", "Does not auto-edit GTFS or prove outside approval."),
		cockpitAction("share_public_urls", "Share public URLs", cockpitShareURLsStatus(page), cockpitShareURLsSignal(page), "Share URLs", "/admin/operations/feeds", "Ask an administrator before changing the public root, HTTPS, DNS, or off-host checks.", "This private view does not show consumer ingestion, listing, display, or final-root readiness."),
		cockpitAction("maintain_system", "Maintain system", maintenanceStatusToCockpit(page.Maintenance.OverallStatus), page.Maintenance.OverallStatus, "Maintain", "/admin/operations/maintenance", "Ask an administrator for backup/restore, support bundles, upgrades, or external diagnostics.", "This private view does not show support, SLA, hosted service, or production readiness."),
	}
}
