...
func buildOperationsCockpit(page operationsPage) operationsCockpitView {
	...
	cards := []operationsCockpitCard{
		...
		cockpitCard("manage_alerts", "Manage alerts", alertStatus(page), feedEvidence(page, "alerts"), "Open the Alerts Console for lifecycle work, then review the configured Alerts feed state.", "/admin/alerts/console", []string{"docs/requirements-calitp-compliance.md"}, "Does not prove consumer display or agency approval."),
