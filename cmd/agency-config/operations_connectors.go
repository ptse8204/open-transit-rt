...
func buildConnectorHub(page operationsPage) connectorHubView {
	categories := []connectorCategory{
		...
		connectorCategoryView(
			...
			"Extension policy does not prove runtime distribution support, third-party compatibility, compliance, consumer acceptance, or production readiness.",
		),
	}
...
func connectorCatalogRows() []connectorCatalogRow {
	return []connectorCatalogRow{
		...
		connectorCatalogRowView(
			...
			"available",
			"Server-owned static GTFS validator tooling when installed by a technical helper.",
		...
		connectorCatalogRowView(
			...
			"available",
			"Server-owned GTFS Realtime validator wrapper when installed by a technical helper.",
