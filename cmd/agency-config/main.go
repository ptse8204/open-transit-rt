...
func newHandlerWithRealtimeAndPasswordStore(agencyID string, scheduleBuilder scheduleBuilder, store publicationStore, deviceStore devices.Store, ready pinger, admin adminAuth, realtime realtimeArtifactSource, suppliedPasswordStore passwordAuthStore) http.Handler {
	...
	mux.Handle("/admin/operations/feed-health", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "no-store")
		adminRead(http.HandlerFunc(h.operationsRoot)).ServeHTTP(w, r)
	}))
	mux.Handle("/admin/operations/feed-health.json", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "no-store")
		adminRead(http.HandlerFunc(h.operationsRoot)).ServeHTTP(w, r)
	}))
	...
	mux.Handle("/admin/operations/readiness.json", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "no-store")
		adminRead(http.HandlerFunc(h.operationsRoot)).ServeHTTP(w, r)
	}))
	...
	mux.Handle("/admin/operations/prediction-lab", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "no-store")
		adminRead(http.HandlerFunc(h.operationsRoot)).ServeHTTP(w, r)
	}))
	mux.Handle("/admin/operations/prediction-lab.json", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "no-store")
		adminRead(http.HandlerFunc(h.operationsRoot)).ServeHTTP(w, r)
	}))
	mux.Handle("/admin/operations/connectors/tests", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "no-store")
		adminRead(http.HandlerFunc(h.operationsRoot)).ServeHTTP(w, r)
	}))
	mux.Handle("/admin/operations/connectors/tests.json", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "no-store")
		adminRead(http.HandlerFunc(h.operationsRoot)).ServeHTTP(w, r)
	}))
...
func newInternalRealtimeArtifactSource(pool *pgxpool.Pool, agencyID string, fallback realtimeArtifactSource) realtimeArtifactSource {
	...
	adapter, err := prediction.NewAdapterFromEnv(os.LookupEnv, gtfsRepo, ops)
	if err == nil {
