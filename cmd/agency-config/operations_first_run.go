...
func buildOperationsFirstRun(page operationsPage) operationsFirstRunView {
	...
	tasks := []operationsFirstRunTask{
		...
		firstRunTask(
			...
			"Import an agency GTFS ZIP or publish a GTFS Studio draft, then review GTFS quality triage.",
		...
		firstRunTask(
			...
			"Copy or review each URL below; fill missing publication metadata before sharing or validating external-facing paths.",
		...
		firstRunTask(
			...
			"Optional external systems should stay behind sidecar, manifest, or adapter boundaries with synthetic conformance checks first.",
			"Review Connector Hub and connector tests before any deployment-owned external integration.",
