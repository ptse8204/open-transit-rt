...
func buildOperationsFirstRun(page operationsPage) operationsFirstRunView {
	...
	tasks := []operationsFirstRunTask{
		...
		firstRunTask(
			...
			checklistStatusNeedsReview,
			"Operator smoke, deployment doctor, reliability, and support-bundle checks are linked as private operator-run diagnostics.",
			"These checks help separate local demo behavior, target deployment review, and retained evidence intake before any outside statement is made.",
			"Run documented diagnostics only in the operator environment, review redaction, and keep maintenance decisions outside this GET-only page.",
			"/admin/operations/reliability",
			"docs/tutorials/small-agency-maintenance-guide.md",
			"Maintenance and support checks do not prove SLA coverage, managed support commitment, deployment ownership, public launch, or approval.",
		),
	}
