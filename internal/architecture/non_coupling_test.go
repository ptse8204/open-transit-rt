...
func TestTripUpdatesDoesNotCoupleIntoExistingPhaseEntrypoints(t *testing.T) {
	for _, pkg := range []string{
		...
		for _, dep := range deps {
			switch dep {
			case "open-transit-rt/internal/prediction",
				"open-transit-rt/internal/feed/tripupdates",
				"open-transit-rt/internal/alerts",
...
func TestAlertsDoNotCoupleIntoPredictionOrTripUpdates(t *testing.T) {
	for _, pkg := range []string{
		"open-transit-rt/internal/prediction",
		"open-transit-rt/cmd/feed-trip-updates",
	} {
		output, err := exec.Command("go", "list", "-deps", pkg).CombinedOutput()
		if err != nil {
			t.Fatalf("go list %s: %v\n%s", pkg, err, output)
		...
		for _, dep := range deps {
			switch dep {
			case "open-transit-rt/internal/alerts", "open-transit-rt/internal/feed/alerts":
				t.Fatalf("%s unexpectedly depends on %s", pkg, dep)
			}
		}
	}
